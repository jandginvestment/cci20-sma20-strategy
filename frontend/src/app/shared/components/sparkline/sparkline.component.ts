...
export class SparklineComponent {
  ...
  // Map values (-200 to 200 typical CCI) to 0-20 height
  ...
  positivePath = computed(() => {
    const points = this.mappedPoints();
    if (points.length === 0) return '';
    const h = 10; // Zero line y
    let path = \`M \${points[0].x},\${h} L \`;
    path += points.map(p => \`\${p.x},\${Math.min(p.y, h)}\`).join(' L ');
    path += \` L \${points[points.length-1].x},\${h} Z\`;
    return path;
  });
  ...
  negativePath = computed(() => {
    ...
    const h = 10; // Zero line y
    let path = \`M \${points[0].x},\${h} L \`;
    path += points.map(p => \`\${p.x},\${Math.max(p.y, h)}\`).join(' L ');
    path += \` L \${points[points.length-1].x},\${h} Z\`;
    return path;
  });
}
